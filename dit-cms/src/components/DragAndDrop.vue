    ...
    <div class="panel panel-vue padding" v-if="item.children">
      ...
      <div class="panel-body">
        <ul v-dnd-list
          ...
          <list v-for="(col, index) in item.children" :item="col" :list="item.children" :index="index" :selected.sync="selected" :disable.sync="disable"></list>
        </ul>
      </div>
    </div>
    <p v-else>
...
export default {
  ...
  computed: {
  isFolder: function () {
    return this.item.data.pageTitle &&
      this.item.data.pageTitle.length > 0
  },
  isEnglish: function () {
    return this.english
  }
